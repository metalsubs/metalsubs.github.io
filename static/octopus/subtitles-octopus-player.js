var SubtitlesOctopus = function (options) {
    ...
    self.init = function () {
        ...
        self.createCanvas();
        self.setVideo(options.video, options.player);
    ...
    self.setVideo = function (video, player) {
      self.video = video;
      self.player = player;    
      if (self.video) {
            var timeupdate = function () {
                self.setCurrentTime(player.currentTime() + self.timeOffset);
            }
            self.player.on("timeupdate", timeupdate);
            self.player.on("playing", function () {
                self.setIsPaused(false, player.currentTime() + self.timeOffset);
            });
            self.player.on("pause", function () {
                self.setIsPaused(true, player.currentTime() + self.timeOffset);
            });
            ...
            self.player.on("ratechange", function () {
